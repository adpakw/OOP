...
const Vector Item::get_position() const {
    return position;
}

void Item::set_position(const Vector &value) {
    position = value;
}

const Vector Item::get_size() const {
    return size;
}
...
bool Item::isCollision(const Item &item) const {
    return
