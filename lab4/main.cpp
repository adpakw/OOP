...
int main() {
  ...
  assert(
      graph.length() ==
      0); //размер коллекции, созданной конструктором по умолчанию, равен нулю

  PostOffice postOffice1 = PostOffice(1, 0);
  PostOffice postOffice2 = PostOffice(2, 1);
  PostOffice postOffice3 = PostOffice(3, 3);

  GraphNode<int, PostOffice> node1 =
      GraphNode<int, PostOffice>(postOffice1.getIndex(), postOffice1);
  GraphNode<int, PostOffice> node2 =
      GraphNode<int, PostOffice>(postOffice2.getIndex(), postOffice2);
  GraphNode<int, PostOffice> node3 =
      GraphNode<int, PostOffice>(postOffice3.getIndex(), postOffice3);

  assert(node1.getKey() == 1);
  assert(node1.getValue().getMails() == 0);
  graph.add(node1);
  assert(graph.length() ==
         1); //при добавлении нового значения размер коллекции, реализованной на
             //множествах, увеличивается на единицу, а при добавлении имеющегося
             //значения размер остаётся прежним;
  graph.add(node1);
  graph.add(node2);
  ...
  assert(
      graph ==
      emptyGraph); //непустая коллекция после удаления всех объектов равна
  ...
  assert(graph3 ==
         emptyGraph); //при пересечении любой коллекции с пустой, оператор &&
                      //возвращает пустую коллекцию (any && empty == empty);
  assert(graph2 ==
         graph2); //при пересечении одинаковых коллекций, оператор && возвращает
                  //новую коллекцию, равную любой из них (any && any == any);
  ...
  graph.add(node3);
  assert(graph !=
         graphCopy); //копия непустой коллекции равна оригиналу, после вставки,
                     //удаления или изменения элемента равенство нарушается;
  ...
  assert(graph5 == graph5Copy); //непустая коллекция, сохранённая в файл, равна
                                //новой коллекции, загруженной из того же файла.
  ...
  GraphNode<int, PostOffice> pathGN1 =
      GraphNode<int, PostOffice>(pathPO1.getIndex(), pathPO1);
  GraphNode<int, PostOffice> pathGN2 =
      GraphNode<int, PostOffice>(pathPO2.getIndex(), pathPO2);
  GraphNode<int, PostOffice> pathGN3 =
      GraphNode<int, PostOffice>(pathPO3.getIndex(), pathPO3);
  GraphNode<int, PostOffice> pathGN4 =
      GraphNode<int, PostOffice>(pathPO4.getIndex(), pathPO4);
  GraphNode<int, PostOffice> pathGN5 =
      GraphNode<int, PostOffice>(pathPO5.getIndex(), pathPO5);
  GraphNode<int, PostOffice> pathGN6 =
      GraphNode<int, PostOffice>(pathPO6.getIndex(), pathPO6);
  ...
  std::vector<GraphNode<int, PostOffice>> path =
      graphPath.path(pathGN1, pathGN5);
